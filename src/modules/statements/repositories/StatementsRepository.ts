...
export class StatementsRepository implements IStatementsRepository {
	...
	async transfer ({ sender_id, recipient_id, amount, description }: IMakeTransferDTO): Promise<Statement[]> {
		const senderTransferOperation = this.repository.create({
			user_id: sender_id,
			recipient_id,
			amount,
			description,
		...
		await this.repository.save(senderTransferOperation);
		await this.repository.save(recipientTransferOperation);

		return [senderTransferOperation, recipientTransferOperation];
	}
}
